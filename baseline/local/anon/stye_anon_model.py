...
def conv1d(in_planes, out_planes, kernel_size=3, stride=1, padding=1):

    out = Dynamic_conv1d(in_planes=in_planes,
...
class AdaIN(nn.Module):
    ...
    def __init__(self, num_features, style_dim, eps=1e-05, momentum=0.1):
        ...
        self.norm = nn.InstanceNorm1d(num_features, eps=eps, momentum=momentum, affine=False, track_running_stats=True)
    ...
    def forward(self, x, s):        
        ...
        self.gamma = self.linear_scaling(s)
        # print('done')
        self.gamma = self.gamma.view(batch_size, -1, 1)
        self.beta = self.linear_shifting(s).view(batch_size, -1, 1)
        out = self.norm(x)
...
class ResidualBlockAdaIN(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, style_num):
        ...
        self.conv = conv1d(dim_in, dim_out)
...
class StyleEncoder(nn.Module):
    #origin n_intermediates=5
    def __init__(self, input_size=512, ch=4, n_intermediates=2, style_dim=50):
        ...
        activation_height = int(ch / (2 ** n_intermediates))
        lastlayer_dim = int(input_size / (2 ** n_intermediates))
        # layers.append(PrintLayer())
        layers += [
            nn.LeakyReLU(negative_slope=0.1),
            nn.Conv1d(ch, activation_height, kernel_size=1, stride=1, padding=0), # Conv4x4 4->1
            nn.LeakyReLU(negative_slope=0.1),
        ]
        # layers.append(PrintLayer())

        self.conv = nn.Sequential(*layers)
        self.linear = nn.Linear(lastlayer_dim * activation_height , style_dim)
    ...
    def forward(self, x):
        ...
        out = self.linear(x)
        # print(out.shape)
        # print("Done style encoder---------------")
        return out

class MappingNetwork(nn.Module):
    def __init__(self, latent_dim=16, style_dim=50, num_domains=2):
        super().__init__()
        layers = []
...
class Generator(nn.Module):
    """Generator network."""
    #origin repeat_num = 6
    def __init__(self, conv_dim=128, style_dim=50, repeat_num=4):
        super(Generator, self).__init__()

        num_of_inputs_list = []
        layers = []
        layers.append(nn.Conv1d(1, conv_dim, kernel_size=3, padding=1, bias=False))
        ...
        layers.append(nn.Conv1d(curr_dim, 1, kernel_size=7, stride=1, padding=3, bias=False))
        num_of_inputs_list += [1]
...
class Discriminator(nn.Module):
    ...
    # origin repeat_num = 5
    def __init__(self, input_size=512, conv_dim=2, repeat_num=3):
        super(Discriminator, self).__init__()
        layers = []
        # layers.append(PrintLayer())
        layers.append(nn.Conv1d(1, conv_dim, kernel_size=4, stride=2, padding=1))
        ...
        kernel_size = int(input_size / np.power(2, repeat_num))  # 1
        # layers.append(PrintLayer())
        self.main = nn.Sequential(*layers)
        self.conv_dis = nn.Conv1d(curr_dim, 1, kernel_size=kernel_size, stride=1, padding=0,
                                  bias=False)  # padding should be 0

    def forward(self, x):
        h = self.main(x)
        out_src = self.conv_dis(h)
        # print("done disciminator-----------")
