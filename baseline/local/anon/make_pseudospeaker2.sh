...
if [ $stage -le 0 ]; then
  ...
  echo "pool_data: ${pool_data}"
  echo "xvec_out_dir: ${xvec_out_dir}"
  echo "pseudo_xvecs_dir: ${pseudo_xvecs_dir}"
  echo "src_xvec_dir: ${src_xvec_dir}"
  echo "rand_level: ${rand_level}"
  echo "cross_gender: ${cross_gender}"
  echo "rand_seed: ${rand_seed}"
  echo "threshold: ${threshold}"
  echo "combine_genders: ${combine_genders}"
  python local/anon/gen_pseudo_xvecs3.py ${src_data} ${pool_data} \
	  ${xvec_out_dir} ${pseudo_xvecs_dir} ${src_xvec_dir} ${rand_level} \
    ${cross_gender} ${rand_seed}\
