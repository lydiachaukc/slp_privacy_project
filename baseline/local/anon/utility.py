...
speakers = CommonInfo('dataset/vcc2020/speakers/').speakers


class Normalizer(object):
    '''Normalizer: convience method for fetch normalize instance'''
    def __init__(self, statfolderpath: str='dataset/etc_256/'):
...
class GenerateStatistics(object):
    def __init__(self, folder: str ='dataset/processed_256/'):
        self.folder = folder
        ...
        for s in speakers:
            ...
            for one_file in os.listdir(os.path.join(folder, s)):
    ...
    def generate_stats(self, statfolder: str = 'dataset/etc_256/'):
        ...
        for one_speaker in self.include_dict_npz.keys():
            ...
            for one_file in arr01:
                t = np.load(os.path.join(self.folder, one_speaker, one_file))
    ...
    def normalize_dataset(self):
        ...
        for speaker in speakers:
            files = librosa.util.find_files(os.path.join(self.folder, speaker), ext='npy')
